...
def dispatch_signal(symbol, price, sig, ind, engine_type, daily_note,
    ...
    msg = (
        ...
        f"     {desc}\n"
        f"📊  Score    <code>{bar}</code>  {score}/{score_max}\n"
        f"{SEP}\n"
        f"🛑  SL    <code>${sl:.6f}</code>    {((sl-price)/price*100):+.2f}%\n"
        f"🎯  TP1   <code>${tp1:.6f}</code>   +{((tp1-price)/price*100):.1f}%  ·  {rr1:.1f}R\n"
        f"🎯  TP2   <code>${tp2:.6f}</code>   +{((tp2-price)/price*100):.1f}%  ·  {rr2:.1f}R\n"
        f"🎯  TP3   <code>${tp3:.6f}</code>   +{((tp3-price)/price*100):.1f}%  ·  {rr3:.1f}R\n"
        f"{SEP}\n"
        f"💼  {pos_coins:.4f} {symbol[:-4]}   <code>${pos_usd:,.2f}</code>\n"
        f"⚡  Risk   <code>-${risk_usd:,.2f}</code>   ({user_risk}%)\n"
        f"{SEP}\n"
        f"{btc_line}"
        f"{info_line}\n"
        f"{SEP}\n"
