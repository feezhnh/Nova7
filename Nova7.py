...
def dispatch_signal(symbol, price, sig, ind, engine_type, daily_note,
    ...
    # ─── Condition label transformer — semua engine types ────────────────────
    import re as _re
    def _fmt_cond(key, passed):
        # Pecah High 20-C → Break / Declined
        if 'Pecah High 20-C' in key:
            val  = key.split('(')[1].rstrip(')') if '(' in key else ''
            verb = 'Break' if passed else 'Declined'
            return f"{verb} 20-C ({val})" if val else f"{verb} 20-C"
        # Atas EMA21 → Above / Below
        if key.startswith('Atas EMA21'):
            val  = key.split('(')[1].rstrip(')') if '(' in key else ''
            verb = 'Above' if passed else 'Below'
            return f"{verb} EMA21 ({val})" if val else f"{verb} EMA21"
        # RSI (semua engine) — tunjuk nilai sebenar sahaja
        if key.startswith('RSI') and '[' in key:
            m = _re.search(r'\[(\d+\.?\d*)', key)
            if m:
                return f"RSI {float(m.group(1)):.0f}"
        # Buang [value] bracket untuk semua keys lain
    ...
    if sig.get('conditions'):
        for cname, cpass in sig['conditions'].items():
            label = _fmt_cond(cname, cpass)
            conditions_block += f"{'✅' if cpass else '❌'}  {label}\n"
    ...
    if macro_state:
        ...
        t_thresh   = t.get('macro_btc_24h_min', -2.0)
        ema_ok_    = macro_state.get('btc_above_ema21d', True) if engine_type == 'ACCUMULATION' else True
        status_dot = "🟢" if (macro_state['btc_24h_pct'] >= t_thresh and ema_ok_) else "🔴"
        btc_line   = f"BTC: {macro_state['btc_24h_pct']:+.2f}% {btc_arr}  24h Status: {status_dot}\n"

    # ─── MESSAGE FORMAT (LOCKED) — berlaku untuk SEMUA engine ────────────────
    msg = (
        ...
        f"{btc_line}"
        f"{SEP}\n"
        f" RVOL {sig['rvol']:.2f}x  ·  BB {bb_val:.1f}%  ●  Nova7  ●"
    )
...
async def _layer2_sniper_impl(symbol, scan_type, force, chat_id, user_cap, user_risk):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if sig:
            ...
            # ── HYBRID SCORING GATE ──────────────────────────────────────────
            # BREAKOUT 3/5 (60%) = marginal setup, mungkin selepas big move
            # Jangan dispatch terus — masuk M15 RBS queue.
            # BREAKOUT 4/5+ = high conviction → dispatch segera.
            # Semua engine lain (ACCUMULATION, RETEST, SWEEP) → dispatch terus.
            if scan_type == 'BREAKOUT' and sig.get('score', 5) == 3 and not force:
                with _marginal_bo_lock:
                    _marginal_bo_watch[symbol] = {
                        'break_level': sig.get('break_level', closes[-1]),
                        'sig':         sig,
                        'daily_note':  daily_note,
                        'user_cap':    user_cap,
                        'user_risk':   user_risk,
                        'expiry':      time.time() + 14400,  # 4h window
                    }
                log_activity(f"{symbol} 🟡 3/5 → M15 RBS queue (nanti konfirmasi)")
            ...
            # Sebelum: "SAHARAUSDT ❌ No Fresh Lower Low (body)"
            # Sekarang: "SAHARAUSDT ❌ 2/4 — No Fresh Lower Low"
...
# ==========================================
# M15 RBS MONITOR — Hybrid Scoring Confirmation Worker
# ==========================================
async def m15_rbs_monitor():
    """
    Jalan setiap 3 minit. Monitor BREAKOUT 3/5 dalam _marginal_bo_watch.

    Dispatch bila SEMUA tiga syarat ini dipenuhi:
      1. Harga pullback ke break level ±2% (RBS zone)
      2. M15 struktur bullish — BOS ↑ atau CHoCH ↑
      3. M15 reversal candle — Hammer / Engulfing / Morning Star  +  RVOL ≥ 1.3x

    Bila confirmed: score dinaik ke 4/5 (80%), signal dispatch dengan tag ✓M15.
    Bila expiry 4h tanpa konfirmasi: buang dari queue, apply RT_ cooldown.
    """
    logger.info("✅ [M15-RBS] Hybrid scoring monitor started.")
    while True:
        await asyncio.sleep(180)
        try:
            now = time.time()
            with _marginal_bo_lock:
                stale = [s for s, d in _marginal_bo_watch.items() if now > d['expiry']]
                for s in stale:
                    del _marginal_bo_watch[s]
                    save_cooldown(f"RT_{s}", 2)
                    log_activity(f"{s} ⏰ M15 RBS expired (3/5 unconfirmed)")
                candidates = list(_marginal_bo_watch.items())
            ...
            for sym, meta in candidates:
                if check_cooldown(sym):
                    with _marginal_bo_lock:
                        _marginal_bo_watch.pop(sym, None)
                    continue
                ...
                if not cur:
                    continue
                cur_price   = cur['c']
                break_level = meta['break_level']

                # Syarat 1: dalam ±2% dari break level (RBS zone)
                dist_pct = (cur_price - break_level) / break_level * 100
                if not (-2.0 <= dist_pct <= 2.0):
                    continue

                # Syarat 2 & 3: M15 struktur + reversal candle
                try:
                    async with aiohttp.ClientSession() as session:
                        klines_m15 = await _smc_instance._fetch_m15(sym, session)
                        if not klines_m15 or len(klines_m15) < 10:
                            continue

                        bos      = _smc_instance._detect_bos_choch(klines_m15)
                        bc_type  = bos.get('type', 'NEUTRAL')
                        bull_struct = bc_type in ('CHOCH_BULL', 'BOS_BULL')
                        ...
                        pattern  = CandlePatternDetector().detect(
                            m15_o, m15_c, m15_h, m15_l, lookback=3)
                        bull_pat = (pattern.get('found') and
                                    pattern.get('pattern') in
                                    ('HAMMER', 'ENGULFING', 'MORNING STAR'))

                        avg_v    = sum(m15_v[-21:-1]) / 20 if len(m15_v) >= 21 else 1
                        vol_ok   = (m15_v[-1] / avg_v if avg_v > 0 else 0) >= 1.3

                        if not ((bull_struct or bull_pat) and vol_ok):
                            continue

                        # ─── Confirmed ───────────────────────────────────────
                        pat_label  = pattern.get('pattern', '') if bull_pat else ''
                        bos_label  = bos.get('label', '') if bull_struct else ''
                        conf_label = ' + '.join(filter(None, [bos_label, pat_label]))
                        log_activity(f"{sym} ✅ M15 RBS: {conf_label} → dispatch 4/5")

                        # Re-fetch H1 data terkini
                        url = (f"https://api.binance.com/api/v3/klines"
                               f"?symbol={sym}&interval=1h&limit=100")
                        async with session.get(
                                url, timeout=aiohttp.ClientTimeout(total=10)) as r:
                            h1_data = await r.json()
                        if not h1_data or len(h1_data) < 51:
                        ...
                        smc_ctx = await _smc_instance.analyze(sym, ind, session)

                        # Upgrade sig: score 3→4, tambah M15 confirmation condition
                        sig = meta['sig'].copy()
                        sig['smc']   = smc_ctx
                        sig['score'] = 4   # naik dari 3 → 4 selepas M15 confirm
                        sig['conditions'] = dict(sig.get('conditions') or {})
                        sig['conditions'][f"M15 RBS ✓ ({conf_label})"] = True

                        dispatch_signal(sym, h1_c[-1], sig, ind, 'BREAKOUT',
                                        meta['daily_note'],
                                        meta['user_cap'], meta['user_risk'])
                        with _marginal_bo_lock:
                            _marginal_bo_watch.pop(sym, None)

                except Exception as e:
                    logger.debug(f"[M15-RBS] {sym}: {e}")

        except Exception as e:
            logger.error(f"[M15-RBS] Monitor error: {e}")
...
# Hybrid Scoring: BREAKOUT 3/5 → M15 RBS confirmation queue
# Elak FOMO entry selepas big move. Dispatch bila M15 tunjuk RBS + reversal candle.
_marginal_bo_watch = {}  # {symbol: {'break_level', 'sig', 'daily_note', 'user_cap', 'user_risk', 'expiry'}}
_marginal_bo_lock  = threading.Lock()
