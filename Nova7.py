...
def dispatch_signal(symbol, price, sig, ind, engine_type, chart_buf, daily_note,
    ...
    msg += (
        ...
        f"🎯 <b>TP1</b> <code>${tp1:.6f}</code>  +{((tp1-price)/price*100):.1f}%\n"
        f"🎯 <b>TP2</b> <code>${tp2:.6f}</code>  +{((tp2-price)/price*100):.1f}%\n"
        f"🎯 <b>TP3</b> <code>${tp3:.6f}</code>  +{((tp3-price)/price*100):.1f}%\n"
        f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
...
def generate_chart_image(symbol, opens, closes, highs, lows, volumes, ema21, ema50, sl, tp1, tp2, tp3):
    ...
    try:
        ...
        # WAJIB cleanup — cegah matplotlib memory leak (80-150MB per chart)
        if fig is not None:
            plt.close(fig)
        plt.close('all')
...
if __name__ == "__main__":
    init_db()
    # Pre-build matplotlib font cache supaya chart pertama tidak spike RAM
    # semasa scan tasks sedang berjalan
    def _prewarm_matplotlib():
        try:
            import matplotlib.font_manager as fm
            _ = fm.fontManager.get_font_names()
            logger.info("✅ [CHART] Matplotlib font cache ready.")
        except Exception:
            pass
