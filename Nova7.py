...
class IncrementalIndicators:
    ...
    def initialize(self, closes, highs, lows, volumes):
        ...
        self.closes, self.highs, self.lows, self.volumes = closes[-100:
            ], highs[-100:], lows[-100:], volumes[-100:]
    ...
    def _update_rsi(self):
        self.rsi = 100.0 if self.avg_loss == 0 else 100 - \
            (100 / (1 + self.avg_gain / self.avg_loss))
...
class BreakoutHunter:
    def check(self, ind, t):
        ...
        close, rvol, recent_high = ind.closes[-1], ind.get_rvol(
        ), ind.get_recent_high()
...
class AccumulationDetective:
    def check(self, ind, t):
        ...
        if all(conditions.values()):
            sig = {'type': 'ACCUMULATION', 'rvol': rvol,
                'bb': bb, 'low': min(ind.lows[-5:])}
            return sig, conditions
        return None, conditions
...
def generate_chart_image(symbol, closes, highs, lows,
                         volumes, ema21, ema50, sig, sl, tp1, tp2, tp3):
    try:
        n = min(60, len(closes))
        df = pd.DataFrame({
        ...
        addplots = []
        if ema21: addplots.append(
    mpf.make_addplot(
        [ema21] * n,
        color='#00BFFF',
         width=1.2))
        if ema50: addplots.append(
    mpf.make_addplot(
        [ema50] * n,
        color='#FFA500',
         width=1.2))
...
def build_keyboard(symbol):
    ...
    # Baris 2: Social & AI
    markup.row(
        InlineKeyboardButton("🐦 Twitter Live",
     url=f"https://x.com/search?q=%24{base}&f=live"),
        InlineKeyboardButton(
    "✨ AI Insight",
     callback_data=f"ai_summary_{symbol}")
    )
...
def dispatch_signal(symbol, price, sig, ind, engine_type,
    ...
    tp1, tp2, tp3 = price + (risk * 2.0), price + \
                             (risk * 3.5), price + (risk * 5.5)
    ...
    emoji, title = (
    "🚀", "BREAKOUT RADAR") if engine_type == 'BREAKOUT' else (
        "🕵️", "ACCUMULATION SNIPER")
    desc = f"Break: <code>${
    sig.get(
        'break_level',
        0):.6f}</code>" if engine_type == 'BREAKOUT' else f"BB Squeeze: {
            sig.get(
                'bb',
                 0):.2f}%"
    ...
    msg = (
        ...
        f"   • <b>Buy: </b> {pos_coins:.4f} {symbol[:-
    4]} (<code>${pos_usd:,.2f}</code>)\n"
        f"   • <b>Max Loss: </b> <code>-${
    risk_usd:,.2f}</code> ({user_risk}%)\n"
        f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
    ...
    t.get(
        'cd_breakout',
        24) if engine_type == 'BREAKOUT' else t.get(
            'cd_accumulation',
             48))
...
def spot_post_mortem(symbol):
    try:
        ...
        reasons = []
        if rvol_now < 1.0: reasons.append(
    f"🩸 <b>Volume Trap:</b> RVOL {
        rvol_now:.2f}x")
        if btc_change < -1.5: reasons.append(
    f"📉 <b>Macro Drag:</b> BTC {
        btc_change:.2f}%")
    ...
    while True:
        ...
        try:
            async with websockets.connect(url, ping_interval=20, max_size=10**7) as ws:
                logger.info(
                    "✅ [RADAR] Layer 1 Connected. Scanning Mid-Caps...")
                if bot and ADMIN_CHAT_ID:
                    bot.send_message(
    ADMIN_CHAT_ID,
    "🟢 <b>HELLO, NOVA7 NOW ACTIVE.</b>\n2-Layer Radar Online.",
     parse_mode="HTML")
                while True:
                    ...
                    if now - last_pulse >= 300:
                        logger.info(
    f"💓 [PULSE] Radar: {
        pulse_stats['seen']} coins | Promoted: {
            pulse_stats['promoted']} | Signals: {
                stats['signals_sent']} | Rejected: {
                    stats['rejected']}")
                        if activity_log:
                            logger.info(
                                f"📋 [RECENT] {' | '.join(activity_log[-5:])}")
                        last_pulse = now
                        pulse_stats = {'promoted': 0, 'seen': 0}
                    ...
                    for tk in tickers:
                        ...
                        if len(
    radar_history[sym]) > 15: radar_history[sym].pop(0)

                        if len(radar_history[sym]
                               ) >= 6 and sym not in layer2_queue:
                            ...
                            if past_c > 0:
                                ...
                                if change >= momentum and q > min_vol:
                                    ...
                                    log_activity(
                                        f"{sym} ↑{change:.1f}% → Layer 2")
                                    asyncio.create_task(
                                        layer2_sniper(sym, 'BREAKOUT'))
...
def generate_chart_image(symbol, closes, highs, lows,
                         volumes, ema21, ema50, sl, tp1, tp2, tp3):
    try:
        n = min(60, len(closes))
...
async def layer2_sniper(symbol, scan_type, force=False,
                        chat_id=None, user_cap=1000.0, user_risk=2.0):
    try:
        t = get_tuning()
        ...
        # Kira SL/TP (Atau Dummy jika signal invalid untuk tujuan visualisasi
        # carta)
        if sig:
            sl = sig['low'] * 0.995
            risk = closes[-1] - sl
    ...
    tp2,
     tp3)
        ...
        if sig:
            ...
            if daily_filter_on:
                daily_ok, daily_note = check_daily_confluence(
                    symbol, closes[-1])
            ...
            if not daily_ok and not force:
                ...
                if chat_id and bot:
                    bot.send_message(
    chat_id,
    f"🚫 <b>{symbol}</b> ditolak: {daily_note}",
     parse_mode="HTML")
                return
