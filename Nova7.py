...
KILL_LIST = {
    ...
    'USD1','RLUSD','USDV','CUSD','CRVUSD','ALUSD','MUSD','OUSD','DOLA','RAI',
    'FEI','UST','USTC','USDN','USDW','USDB','USDK','UUSD','U',
    # ── Fiat-pegged ───────────────────────────────────────────────────────────
    'EUR','TRY','BRL','AUD','GBP','JPY',
    'EURS','EURI','EURC','EUROC','EURT','AGEUR','XEUR',
    'CADC','XSGD','CNHT','BIDR','IDRT','TRYB','BRZ',
    # ── Wrapped / Bridged ─────────────────────────────────────────────────────
    'WBTC','WETH','WBNB','WSTETH','WEETH','FRXETH','CBETH',
    'BNSOL','WBETH','WSOL','WMATIC','WAVAX','WDOGE','BTCB','HBTC',
    # ── Liquid Staking Tokens ─────────────────────────────────────────────────
    'STETH','RETH','MSOL','JSOL','STSOL','SCNSOL',
    'SFRXETH','OSETH','SWETH','LSETH','ANKRETH','ANKRBNB','ANKRMATIC',
}
HEAVYWEIGHTS = {
...
DEFAULT_TUNING = {
    ...
    'bo_daily_filter': 1,
    # Accumulation
    # acc_rvol     = HARD GATE minimum (0.8x = floor, bukan scoring)
    # acc_rsi_max  = 40 (ketat — RSI 40-48 adalah mild pullback, bukan oversold sebenar)
    # acc_bb_width = 22% (realistic squeeze pada 1H)
    'acc_bb_width': 22.0,
    'acc_rvol': 0.8,
    'acc_rsi_max': 40,    # ← 48→40: RSI 41-48 terlalu neutral untuk accumulation
    'acc_rsi_min': 25,
    ...
    'confirm_required': 0,
    'pending_expiry_h': 2,
    # SL — sl_atr_mult 1.5→2.0: 1.5x terlalu ketat untuk H1 crypto noise
    # ATR H1 altcoin avg 2-4%, 1.5x = SL 3-6% = terlalu mudah kena noise
    # 2.0x = SL 4-8% = lebih selamat dari whipsaw
    'sl_atr_mult': 2.0,   # ← 1.5→2.0: beri ruang noise H1
    'sl_max_pct': 0.08,
    'fail_cooldown_h': 0.33,
    # HTF alignment — 0=warn sahaja, 1=block signal jika 1D+4H kedua-dua bearish
...
def init_db():
    ...
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        ...
        if bad3 and float(bad3[0]) >= 1.0:
            ...
            logger.warning(f"[DB REPAIR] fail_cooldown_h was {bad3[0]}h → fixed to 0.33h")
        bad4 = conn.execute("SELECT value FROM tuning_params WHERE key='acc_bb_width'").fetchone()
        if bad4 and float(bad4[0]) <= 15.0:
            conn.execute("INSERT OR REPLACE INTO tuning_params VALUES ('acc_bb_width', 22.0)")
            logger.warning(f"[DB REPAIR] acc_bb_width was {bad4[0]}% → fixed to 22.0%")
        # Tight defaults — upgrade jika masih guna nilai lama
        bad5 = conn.execute("SELECT value FROM tuning_params WHERE key='acc_rsi_max'").fetchone()
        if bad5 and float(bad5[0]) >= 45.0:
            conn.execute("INSERT OR REPLACE INTO tuning_params VALUES ('acc_rsi_max', 40.0)")
            logger.warning(f"[DB REPAIR] acc_rsi_max was {bad5[0]} → tightened to 40.0")
        bad6 = conn.execute("SELECT value FROM tuning_params WHERE key='sl_atr_mult'").fetchone()
        if bad6 and float(bad6[0]) <= 1.5:
            conn.execute("INSERT OR REPLACE INTO tuning_params VALUES ('sl_atr_mult', 2.0)")
            logger.warning(f"[DB REPAIR] sl_atr_mult was {bad6[0]} → widened to 2.0")
        # htf_block_on_bear — param baru, init ke 0 jika belum ada
        conn.execute("INSERT OR IGNORE INTO tuning_params VALUES ('htf_block_on_bear', 0)")
...
class BreakoutHunter:
    ...
    def check(self, ind, t):
        ...
        if score >= 3:
            # ── RVOL Floor Gate untuk BREAKOUT ───────────────────────────────
            # RVOL < 0.5x = tiada momentum sebenar di sebalik breakout
            # WLDUSDT 0.22x, BABYUSDT 0.47x — contoh dari journal yang hit SL
            # Ini adalah floor minimum, bukan scoring threshold (1.8x)
            rvol_floor = t.get('bo_rvol', 1.8) * 0.30   # 30% dari threshold = 0.54x
            if rvol < rvol_floor:
                return None, {
                    f"RVOL Floor < {rvol_floor:.2f}x [{rvol:.2f}x — tiada momentum]": False
                }
            # Guna get_structure_low() — swing low terkini, bukan min(-20:) mutlak
            # Lebih tepat untuk SL placement berbanding absolute minimum
            structure_low = ind.get_structure_low(lookback=15) if hasattr(ind, 'get_structure_low') else min(ind.lows[-20:])
            sig = {
...
class AccumulationDetective:
    ...
    def check(self, ind, t):
        ...
        # ── HARD GATE 3: RVOL minimum (volume wajib ada) ─────────────────────
        # RVOL < threshold = tiada minat pasaran = mudah kena manipulation/fakeout
        # Signal dengan RVOL 0.09x-0.19x selalu kena SL — ini punca utama losses
        # Tetap tunable: /tune custom acc_rvol=0.6 untuk longgarkan
        if rvol < rvol_min:
            return None, {
...
class SMCAnalyzer:
    ...
    def _set_cache(self, key, data):
        with _smc_cache_lock:
            ...
            # Buang entries expired — elak cache membesar tanpa had selepas berjam-jam
            now   = time.time()
            stale = [k for k, v in _smc_cache.items() if now - v['t'] > 7200]
            for k in stale:
                del _smc_cache[k]
...
def dispatch_signal(symbol, price, sig, ind, engine_type, daily_note,
    ...
    if engine_type == 'ACCUMULATION':
        ...
        if _macro and _macro['btc_24h_pct'] < -4.0:
            ...
            bump_stat('rejected')
            save_cooldown(symbol, 1.0)
            return

    # ── Filter 5: HTF Alignment ────────────────────────────────────────────────
    if (int(t.get('htf_block_on_bear', 0)) == 1
            and engine_type in ('ACCUMULATION', 'BREAKOUT', 'RETEST')):
        smc_htf = sig.get('smc') or {}
        if (smc_htf.get('htf_1d') == 'BEAR'
                and smc_htf.get('htf_4h') == 'BEAR'):
            logger.info(f"[HTF BLOCK] {symbol} ditolak: 1D+4H bearish")
            log_activity(f"{symbol} ❌ HTF block: 1D 🔴 4H 🔴")
            bump_stat('rejected')
            save_cooldown(symbol, float(t.get('fail_cooldown_h', 0.33)))
            return
...
async def _check_reentry_opportunity(symbol, sl_price, engine):
    """Wrapper dengan semaphore — hadkan 2 concurrent checks semasa crash."""
    ...
    while True:
        ...
        try:
            ...
            for w in watches:
                ...
                # ── Drop: harga dah jauh atas zone (trade missed, zone lapuk) ──
                zone_top = max(
                    w['ob_top']  if w['ob_top']  > 0 else 0,
                    w['fvg_top'] if w['fvg_top'] > 0 else 0
                )
                if zone_top > 0 and cur_price > zone_top * 1.40:
                    drop_reentry_watch(symbol)
                    save_cooldown(symbol, 2.0)
                    log_activity(f"{symbol} 📍 Re-entry zone lapuk (harga dah +40% atas zone) — drop")
                    continue
                ...
                # ── Max M15 check attempts: 12 × 5min = 1 jam ────────────────
                # Jika M15 tidak confirm selepas 1 jam dalam zone → setup gagal
                check_count = w.get('reentry_count', 0)
                if check_count >= 12:
                    drop_reentry_watch(symbol)
                    save_cooldown(symbol, float(get_tuning().get('fail_cooldown_h', 0.33)))
                    log_activity(f"{symbol} 📍 Re-entry M15 tidak confirm selepas 1 jam — drop")
                    continue
                # Increment check count
                with db_lock, sqlite3.connect(DB_NAME) as _conn:
                    _conn.execute(
                        "UPDATE reentry_watchlist SET reentry_count=? WHERE symbol=?",
                        (check_count + 1, symbol))

                log_activity(f"{symbol} 📍 Harga masuk {zone_name} → M15 analisis ({check_count+1}/12)")
            ...
            async with websockets.connect(url, ping_interval=20, max_size=10**7) as ws:
                ...
                while True:
                    ...
                    for tk in tickers:
                        ...
                        # Harga $0.97–$1.03 = hampir pasti pegged asset (UUSDT dll)
                        if 0.97 <= c <= 1.03:
                            continue
...
async def _layer2_sniper_impl(symbol, scan_type, force, chat_id, user_cap, user_risk):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if sig:
            # Daily confluence — gate aktif jika bo_daily_filter=1
            # Tunable: /tune custom bo_daily_filter=0 untuk matikan
            ...
            # Block signal jika daily confluence gagal (kecuali force mode)
            if daily_filter_on and not daily_ok and not force:
                ...
                if chat_id and bot:
                    bot.send_message(chat_id,
                        f"🚫 <b>{symbol}</b> ditolak Daily: {daily_note}",
                        parse_mode="HTML")
                return
...
def cmd_tune(msg):
    ...
    if not args or args[0] == 'show':
        ...
        report = (
            ...
            f"• Daily Filter: <code>{'GATE (block)' if int(t.get('bo_daily_filter', 1)) == 1 else 'OFF'}</code>\n"
            f"<b>🕵️ Accumulation Engine:</b>\n"
            f"• RVOL Hard Gate: <code>≥ {t.get('acc_rvol', 0.8):.2f}x</code> (wajib lulus)\n"
            f"• BB Width max: <code>{t.get('acc_bb_width', 22.0):.1f}%</code>\n"
            f"• RSI range: <code>{t.get('acc_rsi_min', 25):.0f}-{t.get('acc_rsi_max', 40):.0f}</code>\n"
            f"<b>🌐 Macro Filter:</b>\n"
            f"• BTC 24h min: <code>{t.get('macro_btc_24h_min', -2.0):+.1f}%</code>\n"
            f"• Circuit Breaker: <code>BTC &lt; -4.0% → ACCUM STOP</code>\n"
            f"<b>📐 HTF Alignment:</b>\n"
            f"• htf_block_on_bear: <code>{'BLOCK (1D+4H bearish → tolak)' if int(t.get('htf_block_on_bear', 0)) == 1 else 'WARN sahaja — set 1 untuk block'}</code>\n"
            f"<b>📐 SL Calculation:</b>\n"
            f"• ATR multiplier: <code>{t.get('sl_atr_mult', 2.0):.1f}x</code>\n"
            ...
            f"• Fail cooldown: <code>{t.get('fail_cooldown_h', 0.33):.2f}h</code>\n"
            f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
            f"💡 <code>/tune custom key=value</code>\n"
            f"Longgar: <code>acc_rsi_max=48 acc_rvol=0.6 bo_daily_filter=0</code>\n"
            f"Ketat: <code>htf_block_on_bear=1 acc_rsi_max=35 sl_atr_mult=2.5</code>"
        )
    ...
    if args[0] == 'standard':
        ...
        bot.reply_to(msg,
            "⚖️ <b>Mode STANDARD aktif.</b>\n\n"
            "RVOL gate: 0.8x  ·  RSI: 25-40\n"
            "Daily filter: AKTIF  ·  HTF block: OFF\n"
            "Circuit breaker: BTC &lt;-4%\n"
            "ATR SL: 2.0x  ·  1 trade per coin\n\n"
            "Sesuai: Pasaran normal / sideways.",
            parse_mode="HTML")

    elif args[0] == 'longgar':
        set_tuning({
            'mode': 1,
            # Breakout — lebih rendah threshold
            'bo_rvol': 1.3, 'bo_rsi_min': 45, 'bo_rsi_max': 80,
            'bo_daily_filter': 0,          # daily filter OFF — catch momentum signals
            # Accumulation — lebar untuk catch early reversal
            'acc_bb_width': 28.0,
            'acc_rvol': 0.5,               # RVOL gate rendah — kadang2 market fly tanpa volume
            'acc_rsi_max': 50, 'acc_rsi_min': 18,
            'acc_require_higher_low': 0,
            # Radar — lebih sensitive
            'radar_momentum': 1.5, 'radar_min_vol': 8_000_000,
            # Cooldown — pendek supaya catch lebih banyak
            'cd_breakout': 12, 'cd_accumulation': 24,
            # Macro — longgar
            'macro_btc_filter': 1, 'macro_btc_24h_min': -5.0,
            'confirm_required': 0,
            # SL — lebih ketat untuk compensate gate yang longgar
            'sl_atr_mult': 1.5, 'sl_max_pct': 0.10,
            'fail_cooldown_h': 0.25,
            # Gates — semua OFF
            'htf_block_on_bear': 0,        # HTF tidak block
        })
        bot.reply_to(msg,
            "🟢 <b>Mode LONGGAR aktif.</b>\n\n"
            "RVOL gate: 0.5x  ·  RSI: 18-50\n"
            "Daily filter: OFF  ·  HTF block: OFF\n"
            "Circuit breaker: BTC &lt;-5%\n"
            "ATR SL: 1.5x  ·  1 trade per coin\n\n"
            "Sesuai: Bull market / altseason.\n"
            "⚠️ Lebih banyak signal — sizing kecil.",
            parse_mode="HTML")
        ...
        set_tuning({
            'mode': 2,
            # Breakout — kuat
            'bo_rvol': 2.5, 'bo_rsi_min': 55, 'bo_rsi_max': 70,
            'bo_daily_filter': 1,
            # Accumulation — sangat selective
            'acc_bb_width': 15.0,
            'acc_rvol': 1.2,               # volume mesti jelas
            'acc_rsi_max': 36, 'acc_rsi_min': 25,
            'acc_require_higher_low': 1,
            # Radar — hanya momentum kuat
            'radar_momentum': 3.0, 'radar_min_vol': 20_000_000,
            # Cooldown — panjang
            'cd_breakout': 48, 'cd_accumulation': 72,
            # Macro — strict
            'macro_btc_filter': 1, 'macro_btc_24h_min': -1.5,
            'confirm_required': 0,
            # SL — lebar untuk elak noise
            'sl_atr_mult': 2.5, 'sl_max_pct': 0.07,
            'fail_cooldown_h': 1.0,
            # Gates — semua ON
            'htf_block_on_bear': 1,        # block jika 1D+4H bearish
        })
        bot.reply_to(msg,
            "🔴 <b>Mode KETAT aktif.</b>\n\n"
            "RVOL gate: 1.2x  ·  RSI: 25-36\n"
            "Daily filter: AKTIF  ·  HTF block: AKTIF\n"
            "Circuit breaker: BTC &lt;-1.5%\n"
            "ATR SL: 2.5x  ·  1 trade per coin\n\n"
            "Sesuai: Bear market / high uncertainty.\n"
            "Signal jarang — tapi berkualiti tinggi.",
            parse_mode="HTML")
