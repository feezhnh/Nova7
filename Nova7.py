...
def init_db():
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS watchlist
                        (symbol TEXT PRIMARY KEY, scan_type TEXT, sig_json TEXT,
                         entry REAL, sl REAL, tp1 REAL, tp2 REAL, tp3 REAL,
                         daily_note TEXT, user_cap REAL, user_risk REAL,
                         added_time REAL, expire_time REAL)''')
...
# ==========================================
def save_watchlist(symbol, scan_type, sig, entry, sl, tp1, tp2, tp3, daily_note, user_cap, user_risk, expire_hours=2):
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        conn.execute('''INSERT OR REPLACE INTO watchlist VALUES
                        (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                     (symbol, scan_type, json.dumps(sig), entry, sl, tp1, tp2, tp3,
                      daily_note, user_cap, user_risk,
                      time.time(), time.time() + (expire_hours * 3600)))
...
class IncrementalIndicators:
    def __init__(self):
        self.closes, self.highs, self.lows, self.volumes, self.opens = [], [], [], [], []
    ...
    def initialize(self, closes, highs, lows, volumes, opens=None):
        if len(closes) < 51: return False
        self.closes = closes[-100:]
        self.highs = highs[-100:]
        self.lows = lows[-100:]
        self.volumes = volumes[-100:]
        self.opens = opens[-100:] if opens else []

        # FIX: EMA21 warm-up bermula dari index 21, EMA50 dari index 50 (berasingan)
        ...
        # RSI Wilder Smoothed (formula asal)
    ...
    def get_bb_width(self):
        ...
        sma = sum(recent) / 20
        std = (sum((p - sma) ** 2 for p in recent) / 19) ** 0.5  # FIX: sample std dev (N-1)
    ...
    def get_atr(self, period=14):
        """Average True Range — untuk SL dinamik"""
        if len(self.closes) < period + 1: return 0.0
        trs = []
    ...
    def get_last_candle_bullish(self):
        """True = candle hijau (close >= open), False = candle merah"""
        if not self.opens or not self.closes: return True
...
class BreakoutHunter:
    def check(self, ind, t):
        ...
        if all(conditions.values()):
            sig = {'type': 'BREAKOUT', 'rvol': rvol, 'break_level': recent_high,
                   'low': min(ind.lows[-20:]), 'atr': ind.get_atr()}
...
class AccumulationDetective:
    def check(self, ind, t):
        ...
        if all(conditions.values()):
            sig = {'type': 'ACCUMULATION', 'rvol': rvol, 'bb': bb,
                   'low': min(ind.lows[-20:]), 'atr': ind.get_atr()}
            return sig, conditions
        return None, conditions
...
# ==========================================
def check_daily_confluence(symbol, current_price, engine_type='BREAKOUT'):
    try:
        ...
        low_20d = min(lows[-20:])
        # Syarat A: harga atas Daily EMA50 — berlaku untuk semua engine
        if current_price > ema50_d:
            return True, f"Above Daily EMA50 ({ema50_d:.6f})"
        # Syarat B: dekat support 20D — HANYA untuk ACCUMULATION (bukan Breakout)
        if engine_type == 'ACCUMULATION' and current_price < low_20d * 1.03:
...
def dispatch_signal(symbol, price, sig, ind, engine_type, chart_buf, daily_note, user_cap, user_risk, force=False):
    if not bot or not TELEGRAM_CHAT_ID or (not force and check_cooldown(symbol)): return

    # SL: ambil yang lebih selamat antara low-20 dan ATR×1.5
    low20_sl = sig['low'] * 0.995
    atr = sig.get('atr', 0)
    atr_sl = price - (atr * 1.5) if atr > 0 else low20_sl
    sl = min(low20_sl, atr_sl)  # ambil lebih rendah = lebih banyak buffer
    ...
    is_bullish_candle = ind.get_last_candle_bullish()
    vol_label = "🟢 <b>Vol: BULLISH</b>" if is_bullish_candle else "⚠️ <b>Vol: CAUTION (Bearish Candle)</b>"
...
async def layer2_sniper(symbol, scan_type, force=False, chat_id=None, user_cap=1000.0, user_risk=2.0):
    try:
        ...
        if not ind.initialize(closes, highs, lows, volumes, opens): return
        ...
        if sig:
            ...
            if daily_filter_on:
                daily_ok, daily_note = check_daily_confluence(symbol, closes[-1], scan_type)
            ...
            # VOLUME DIRECTION ROUTING
            is_bullish_candle = ind.get_last_candle_bullish()

            if is_bullish_candle or force:
                # Candle hijau atau /force → dispatch terus ke channel
                dispatch_signal(symbol, closes[-1], sig, ind, scan_type, chart_buf, daily_note, user_cap, user_risk, force=force)
                stats['signals_sent'] += 1
            else:
                # Candle merah → masuk Watchlist, tunggu konfirmasi rebound
                save_watchlist(symbol, scan_type, sig, closes[-1], sl, tp1, tp2, tp3, daily_note, user_cap, user_risk)
                log_activity(f"{symbol} ⏳ Bearish candle → Watchlist (tunggu rebound)")
                logger.info(f"⏳ [WATCHLIST] {symbol} masuk watchlist — menunggu konfirmasi rebound.")
        else:
            ...
            if chat_id and bot and conditions:
                ...
                # TIDAK VALID: Hantar teks diagnostik ke bot sahaja (private channel kekal eksklusif)
...
# ==========================================
# WATCHLIST MONITOR — TUNGGU KONFIRMASI REBOUND
# ==========================================
async def watchlist_monitor():
    """
    Setiap 30 minit, semak semua symbol dalam watchlist.
    Jika candle terbaru HIJAU + harga masih atas SL → dispatch signal sebagai CONFIRMED REBOUND.
    Jika harga bawah SL atau masa luput → buang senyap.
    """
    while True:
        await asyncio.sleep(1800)  # semak setiap 30 minit
        items = get_watchlist()
        if not items:
            continue
        logger.info(f"⏳ [WATCHLIST] Menyemak {len(items)} item...")
        for item in items:
            symbol = item['symbol']
            try:
                now = time.time()

                # Buang jika sudah tamat tempoh
                if now > item['expire_time']:
                    remove_watchlist_item(symbol)
                    log_activity(f"{symbol} ⌛ Watchlist luput → buang")
                    continue

                # Fetch candle terkini
                url = f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval=1h&limit=100"
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, timeout=10) as resp:
                        data = await resp.json()
                ...
                # Buang jika harga sudah jatuh bawah SL
                if current_price <= saved_sl:
                    remove_watchlist_item(symbol)
                    log_activity(f"{symbol} ❌ Watchlist: SL ditembus → buang senyap")
                    continue

                # Semak candle terkini: adakah hijau?
                last_candle_bullish = closes[-1] >= opens[-1]
                if not last_candle_bullish:
                    log_activity(f"{symbol} ⏳ Watchlist: masih bearish, tunggu...")
                    continue

                # ✅ CONFIRMED REBOUND — Jana carta fresh + dispatch
                ind = IncrementalIndicators()
                if not ind.initialize(closes, highs, lows, volumes, opens):
                    continue

                # Rekonstruk sig dari JSON tersimpan
                sig = json.loads(item['sig_json'])

                # Kira SL/TP fresh berdasarkan harga semasa
                low20_sl = min(lows[-20:]) * 0.995
                atr = ind.get_atr()
                atr_sl = current_price - (atr * 1.5) if atr > 0 else low20_sl
                fresh_sl = min(low20_sl, atr_sl)
                risk = current_price - fresh_sl
                if risk <= 0:
                    remove_watchlist_item(symbol)
                    continue
                fresh_tp1 = current_price + (risk * 2.0)
                fresh_tp2 = current_price + (risk * 3.5)
                fresh_tp3 = current_price + (risk * 5.5)

                # Jana screenshot chart fresh (harga & SL/TP terkini)
                chart_buf = generate_chart_image(
                    symbol, closes, highs, lows, volumes,
                    ind.ema21, ind.ema50,
                    fresh_sl, fresh_tp1, fresh_tp2, fresh_tp3
                )

                # Update sig dengan low terkini untuk dispatch_signal
                sig['low'] = min(lows[-20:])
                sig['atr'] = atr

                confirmed_note = f"✅ CONFIRMED REBOUND | {item['daily_note']}"
                dispatch_signal(
                    symbol, current_price, sig, ind,
                    item['scan_type'], chart_buf,
                    confirmed_note,
                    item['user_cap'], item['user_risk'],
                    force=False
                )
                remove_watchlist_item(symbol)
                log_activity(f"{symbol} 🚀 Watchlist CONFIRMED → Dispatched ke channel")
                logger.info(f"✅ [WATCHLIST] {symbol} confirmed rebound, signal dihantar.")

            except Exception as e:
                logger.error(f"[WATCHLIST] Error semak {symbol}: {e}")
...
if __name__ == "__main__":
    ...
    if RENDER_URL:
        ...
        threading.Thread(target=lambda: asyncio.run(watchlist_monitor()), daemon=True).start()
