...
class AccumulationDetective:
    ...
    def check(self, ind, t):
        ...
        # ── HARD GATE 2: No fresh lower low ──────────────────────────────────
        # 3 candle terkini tidak buat low baru vs 5 candle sebelumnya
        # Elak masuk semasa coin masih dalam selling phase
        if len(ind.lows) >= 8:
            recent_low  = min(ind.lows[-3:])
            prev_low    = min(ind.lows[-8:-3])
            gate_no_new_low = recent_low >= prev_low * 0.985
        else:
            gate_no_new_low = True
        if not gate_no_new_low:
            return None, {
                f"No Fresh Lower Low [{recent_low:.6f} < {prev_low:.6f}]": False
            }
...
class SMCAnalyzer:
    ...
    def _detect_bos_choch(self, klines):
        ...
        # Tentukan konteks sebelum candle semasa
        # (adakah kita dalam uptrend atau downtrend berdasarkan swing structure)
        prev_highs = [p for _, p in sh_list[-4:]]
        prev_lows  = [p for _, p in sl_list[-4:]]

        # Trend sebelumnya: HH+HL = uptrend, LH+LL = downtrend
        was_uptrend   = (len(prev_highs) >= 2 and prev_highs[-1] > prev_highs[-2]
                         and len(prev_lows) >= 2 and prev_lows[-1] > prev_lows[-2])
        was_downtrend = (len(prev_highs) >= 2 and prev_highs[-1] < prev_highs[-2]
                         and len(prev_lows) >= 2 and prev_lows[-1] < prev_lows[-2])
    ...
    async def analyze(self, symbol, ind, session):
        """
        Jalankan semua 4 analisis serentak.
        ...
        try:
            ...
            # M15 BOS/CHoCH (1 API call, cached 5 min)
            klines_m15 = await self._fetch_m15(symbol, session)
            if klines_m15:
                ctx['bos_choch'] = self._detect_bos_choch(klines_m15)

            # FVG + OB dari 1H data yang dah ada dalam ind (tiada API tambahan)
...
def dispatch_signal(symbol, price, sig, ind, engine_type, daily_note,
    ...
    if engine_type == 'SWEEP_REVERSAL':
        ...
        tp1 = sig.get('tp_ema21', price + risk * 1.5)
        tp2 = sig.get('tp_ema50', price + risk * 2.5)
        tp3 = sig.get('tp_swing_high', price + risk * 4.0)
        if tp1 <= price: tp1 = price + risk * 1.5
    ...
    # ── Minimum RR gate — tolak jika TP1 < 1.5R ──────────────────────────────
    # DGBUSDT contoh: TP1 +1.1% = 0.1R → matematiknya rugi walaupun menang
    # Minimum 1.5R bermakna: potensi untung ≥ 1.5× potensi rugi
    MIN_RR1 = 1.5
    if rr1 < MIN_RR1:
        logger.info(f"[RR GATE] {symbol} ditolak: TP1 RR {rr1:.2f} < {MIN_RR1} minimum")
        log_activity(f"{symbol} ❌ RR terlalu rendah ({rr1:.1f}R < {MIN_RR1}R)")
        bump_stat('rejected')
        return
    ...
    if score < score_max and sig.get('conditions'):
        for cname, passed in sig['conditions'].items():
            if not passed:
                short = cname.split('[')[0].strip()[:22]
                short = short.replace('&amp;','&').replace('&lt;','<').replace('&gt;','>')
                short = short.replace('<','').replace('>','').replace('&','').strip()
                failed_line = f"     ✗  {short}\n"
                break
    ...
    info_line = (f"RSI <code>{ind.rsi:.0f}</code>"
                 f"  ·  RVOL <code>{sig['rvol']:.2f}x</code>"
                 f"  ·  BB <code>{bb_val:.1f}%</code>")

    # SMC context — fail-silent, hanya papar jika data ada
    smc_ctx  = sig.get('smc')
    smc_line = ""
    if smc_ctx:
        d1 = "🟢" if smc_ctx['htf_1d'] == 'BULL' else ("🔴" if smc_ctx['htf_1d'] == 'BEAR' else "⚪")
        h4 = "🟢" if smc_ctx['htf_4h'] == 'BULL' else ("🔴" if smc_ctx['htf_4h'] == 'BEAR' else "⚪")
        bc = smc_ctx['bos_choch'].get('label', '—')
        smc_line = f"📐  1D {d1}  4H {h4}  ·  M15 {bc}\n"
        fvg = smc_ctx.get('fvg', {})
        ob  = smc_ctx.get('ob', {})
        if fvg.get('found'):
            smc_line += (f"🌀  FVG  <code>${fvg['bot']:.6f}</code>–"
                         f"<code>${fvg['top']:.6f}</code>"
                         f"  [{fvg['dist_pct']:.1f}%]\n")
        if ob.get('found'):
            zone_tag = "  ✦ DALAM ZONE" if ob.get('in_zone') else ""
            smc_line += (f"📦  OB   <code>${ob['bot']:.6f}</code>–"
                         f"<code>${ob['top']:.6f}</code>{zone_tag}\n")

    SEP = "—" * 17

    msg = (
        f"{emoji}  <b>{cat_short}</b>  <code>{symbol}</code>\n"
        f"{SEP}\n"
        ...
        f"{btc_line}"
        f"{info_line}\n"
        f"{smc_line if smc_line else ''}"
        f"{SEP}\n"
        f"       <b>✦  Nova7  ✦</b>\n"
        f"  {entry_cat}  ·  {coin_cat}  ·  {validity}%"
    )
