    ...
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS breakout_watchlist
            (symbol TEXT PRIMARY KEY,
             ...
             mtf_confluence TEXT,
             status TEXT)''')
        # Rejection Cooldown — cegah coin gagal Layer 2 dari di-scan semula terlalu cepat.
        # Berbeza dari cooldowns (signal success) — ini untuk REJECTION.
        # Struktur: symbol TEXT, until REAL (unix timestamp tamat cooldown), reason TEXT
        conn.execute('''CREATE TABLE IF NOT EXISTS rejection_cooldowns
            (symbol  TEXT PRIMARY KEY,
             until   REAL,
             reason  TEXT)''')
...
# ── Rejection Cooldown helpers ─────────────────────────────────────────────────
# Tujuan: coin yang GAGAL Layer 2 tidak perlu discan semula dalam 20–60 minit.
# Berbeza dari signal cooldown (24H) — ini pendek, untuk elak repeat scan sia-sia.
#
# Tempoh ikut sebab penolakan:
#   RVOL / RSI / teknikal  → 20 minit  (boleh berubah dalam 1 candle)
#   Candle muda            → 15 minit  (tunggu candle matang)
#   MTF BLOCK              → 60 minit  (4H/1D tidak berubah cepat)
#   BTC HARD BLOCK         → 30 minit  (BTC regime cache 5min, so 30min cukup)
#   Daily filter           → 30 minit  (daily berubah perlahan)
#
_REJ_TTL = {
    'RVOL':    20,
    'RSI':     20,
...
def save_rejection_cooldown(symbol, reason_key='TEKNIKAL'):
    minutes = _REJ_TTL.get(reason_key, 20)
    until   = time.time() + (minutes * 60)
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        conn.execute(
            "INSERT OR REPLACE INTO rejection_cooldowns VALUES (?, ?, ?)",
            (symbol, until, reason_key))

def check_rejection_cooldown(symbol):
    """Return True jika coin masih dalam rejection cooldown (skip scan)."""
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        row = conn.execute(
            "SELECT until FROM rejection_cooldowns WHERE symbol=?",
            (symbol,)).fetchone()
        if row and time.time() < row[0]:
            return True
    return False

def clear_rejection_cooldown(symbol):
    """Buang rejection cooldown — dipanggil bila signal berjaya dihantar."""
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        conn.execute(
            "DELETE FROM rejection_cooldowns WHERE symbol=?", (symbol,))

def cleanup_rejection_cooldowns():
    """Buang entry yang dah expired — jalan setiap sejam dari layer1_radar."""
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        conn.execute(
            "DELETE FROM rejection_cooldowns WHERE until < ?", (time.time(),))
...
# ==========================================
async def get_mtf_analysis(symbol, session, ttl=1800):
    """
    MTF analysis dengan cache per-symbol (TTL 30 minit).
    4H + 1D data tidak berubah dalam 30 minit — selamat di-cache.
    """
    global _mtf_cache_lock
    if _mtf_cache_lock is None:
        _mtf_cache_lock = asyncio.Lock()

    async with _mtf_cache_lock:
        now = time.time()
        cached = _mtf_cache.get(symbol)
        if cached and (now - cached['ts']) < ttl:
            return cached['data']

    result = await _get_mtf_analysis_raw(symbol, session)

    async with _mtf_cache_lock:
        _mtf_cache[symbol] = {'data': result, 'ts': time.time()}
        # Buang cache lama — hadkan saiz dict supaya tidak membesar tanpa had
        if len(_mtf_cache) > 300:
            oldest = sorted(_mtf_cache.items(), key=lambda x: x[1]['ts'])
            for sym, _ in oldest[:100]:
                del _mtf_cache[sym]

    return result

async def _get_mtf_analysis_raw(symbol, session):
    """
...
# ── MTF Cache (per-symbol, TTL 30 minit) ──────────────────────────────────────
# KENAPA PERLU CACHE:
# Setiap Layer 2 sniper call fetch 4H(100 candle) + 1D(60 candle) = 2 HTTP request.
# Accumulation scan = 100 coin serentak = 200 HTTP request + 200 array di RAM.
# 4H dan 1D data tidak berubah dalam 30 minit — caching selamat sepenuhnya.
# Tanpa cache: ~160KB data per coin × 100 coin = 16MB tambahan setiap 2 jam.
_mtf_cache = {}          # {symbol: {'data': dict, 'ts': float}}
_mtf_cache_lock = None   # asyncio.Lock — init dalam async context
...
# MEM FIX: Semaphore(3) — lebih selamat dari Semaphore(4) selepas tambah MTF+SMC.
# Sebelum: setiap task = 1H fetch + indicator. Sekarang: 1H + 4H + 1D + SMC arrays.
# Walaupun MTF di-cache, task pertama setiap symbol masih buat 3 HTTP + lebih RAM.
# 3 task × ~120MB peak = ~360MB + base 120MB = 480MB — ada 32MB headroom untuk 512MB.
            ...
            async with websockets.connect(url, ping_interval=20, max_size=10**7) as ws:
                ...
                while True:
                    ...
                    if now - last_pulse >= 300:
                        ...
                        # Cleanup rejection cooldowns yang dah expired (setiap pulse)
                        cleanup_rejection_cooldowns()
                        ...
                        if len(radar_history[sym]) >= 6:
                            ...
                            if past_c > 0:
                                ...
                                if change >= momentum and q > min_vol:
                                    # REJECTION COOLDOWN CHECK — elak scan semula
                                    # coin yang baru ditolak Layer 2
                                    if check_rejection_cooldown(sym):
                                        continue
                                    with queue_lock:
                    ...
                    if now - last_scheduled >= 7200:
                        ...
                        # MEM FIX: kurangkan dari 100 → 50 coin
                        # Setiap task sekarang ada MTF fetch (walaupun cached pada
                        # run pertama semua 50 buat HTTP). RAM lebih selamat.
...
async def layer2_sniper(symbol, scan_type, force=False, chat_id=None,
    ...
    if _layer2_sem is None:
        _layer2_sem = asyncio.Semaphore(3)
        ...
        try:
            ...
            if not force:
                if btc_regime.get('hard_block'):
                    ...
                    bump_stat('rejected')
                    save_rejection_cooldown(symbol, 'BTC')
                    if chat_id and bot:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                # ── CANDLE MATURITY CHECK ─────────────────────────────────
                # Binance klines: d[0] = candle open time (ms), d[6] = candle close time (ms)
                # Maturity = masa berlalu sejak candle buka / 3600 saat
                #
                # KENAPA PENTING:
                #   Candle 1H buka jam 14:00, tutup 14:59:59.
                #   Scan pada 14:03 = hanya 3 minit data = RVOL, RSI, EMA tidak stabil.
                #   Scan pada 14:52 = 52 minit data = hampir finalized, boleh dipercayai.
                #
                # THRESHOLD:
                #   < 10 min (16.7%) → SKIP: terlalu awal, candle baru buka
                #   10–50 min        → ALLOW: data cukup matang
                #   > 50 min (83.3%) → ALLOW dengan nota "near close"
                #
                # Force mode (/force command) bypass check ini sepenuhnya.
                if not force:
                    candle_open_ms  = data[-1][0]   # open time candle semasa (ms)
                    candle_close_ms = data[-1][6]   # close time candle semasa (ms)
                    candle_span_s   = (candle_close_ms - candle_open_ms) / 1000  # should be ~3599s
                    elapsed_s       = (time.time() * 1000 - candle_open_ms) / 1000
                    maturity        = elapsed_s / candle_span_s if candle_span_s > 0 else 1.0
                    maturity        = max(0.0, min(1.0, maturity))  # clamp 0–1

                    if maturity < 0.167:  # < 10 minit
                        mins_elapsed = int(elapsed_s / 60)
                        log_activity(
                            f"{symbol} ⏳ Candle terlalu muda ({mins_elapsed}m) — skip")
                        save_rejection_cooldown(symbol, 'CANDLE')
                        return   # tunggu candle lebih matang
                ...
                if t.get('mtf_filter', 1) == 1 or force:
                    ...
                    if not force and mtf.get('confluence') == 'BLOCK':
                        ...
                        bump_stat('rejected')
                        save_rejection_cooldown(symbol, 'MTF')
                        if chat_id and bot:
                            bot.send_message(chat_id,
                ...
                if sig:
                    ...
                    if not daily_ok and not force:
                        ...
                        bump_stat('rejected')
                        save_rejection_cooldown(symbol, 'DAILY')
                        if chat_id and bot:
                            bot.send_message(chat_id,
                    ...
                    if do_option_a or scan_type == 'ACCUMULATION':
                        ...
                        if msg_id:
                            ...
                            clear_rejection_cooldown(symbol)  # signal berjaya — reset rejection
                    ...
                    # ── 5 syarat teknikal gagal — kenalpasti punca untuk cooldown ──
                    failed = [k for k, v in conditions.items() if not v]
                    if failed:
                        log_activity(
                            f"{symbol} ❌ {failed[0].split('[')[0].strip()[:40]}")
                        bump_stat('rejected')
                        # Pilih reason_key yang paling sesuai untuk cooldown duration
                        f0 = failed[0].upper()
                        if 'RVOL' in f0:
                            save_rejection_cooldown(symbol, 'RVOL')
                        elif 'RSI' in f0:
                            save_rejection_cooldown(symbol, 'RSI')
                        else:
                            save_rejection_cooldown(symbol, 'TEKNIKAL')
                ...
                async with _layer2_sem:
                    try:
                        async with aiohttp.ClientSession() as session:
                            ...
                            # ── Candle maturity check (sama logic Layer 2) ──
                            candle_open_ms  = data[-1][0]
                            candle_close_ms = data[-1][6]
                            candle_span_s   = (candle_close_ms - candle_open_ms) / 1000
                            elapsed_s       = (time.time() * 1000 - candle_open_ms) / 1000
                            maturity        = elapsed_s / candle_span_s if candle_span_s > 0 else 1.0
                            maturity        = max(0.0, min(1.0, maturity))
                            if maturity < 0.167:
                                continue   # candle baru buka, skip coin ini
...
@bot.message_handler(commands=['rejmute'])
def cmd_rejmute(msg):
    """Tunjuk coin dalam rejection cooldown — berguna untuk debug log bersih."""
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        conn.row_factory = sqlite3.Row
        rows = conn.execute(
            "SELECT symbol, until, reason FROM rejection_cooldowns "
            "WHERE until > ? ORDER BY until ASC", (time.time(),)).fetchall()
    if not rows:
        bot.reply_to(msg, "✅ <b>Tiada rejection cooldown aktif.</b>",
                     parse_mode="HTML")
        return
    now   = time.time()
    lines = [f"🔇 <b>REJECTION COOLDOWN ({len(rows)} coin)</b>\n"]
    for r in rows:
        mins_left = int((r['until'] - now) / 60)
        lines.append(f"• <b>{r['symbol']}</b> — {r['reason']} | {mins_left}m lagi")
    bot.reply_to(msg, "\n".join(lines), parse_mode="HTML")
