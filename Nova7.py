...
# ==========================================
# 6. SIMPAN TRADE (DENGAN CONTRACT)
# ==========================================
        ...
        for msg_id, trade in active_items.items():
            ...
            if reply_text:
                try:
                    ...
                    # PASTIKAN SATU BARIS, KURUNG TUTUP
...
def send_welcome(message):
    ...
    bot.reply_to(message, f"⚡ <b>NOVA7 [{status}]</b>\nArahan tersedia: <code>/ca</code>, <code>/scan</code>, <code>/stop</code>, <code>/report</code>, <code>/feedstats</code>, <code>/resetfeedstats</code>", parse_mode="HTML")
...
def cmd_feed_stats(message):
    if str(message.chat.id) != ADMIN_CHAT_ID:
        bot.reply_to(message, "Akses ditolak. Command ini untuk admin sahaja.")
        return
    stats = price_feed.get_stats_summary()
    bot.reply_to(message, f"📊 <b>PRICE FEED STATS</b>\n<pre>{stats}</pre>", parse_mode="HTML")

@bot.message_handler(commands=['resetfeedstats'])
def cmd_reset_feed_stats(message):
    if str(message.chat.id) != ADMIN_CHAT_ID:
        bot.reply_to(message, "Akses ditolak.")
        return
    price_feed.reset_stats()
    bot.reply_to(message, "✅ Stats reset.")
