...
DEFAULT_TUNING = {
    ...
    'acc_bb_width': 15.0,
    'acc_rvol': 0.8,       # FIX ROOT CAUSE: accumulation = quiet vol, 0.8 = floor (not dumping)
    'acc_rsi_max': 48,
    'acc_rsi_min': 25,
...
def init_db():
    ...
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        ...
        if bad and float(bad[0]) > 1.1:
            ...
            logger.warning(f"[DB REPAIR] acc_rvol was {bad[0]} (contradicts BB squeeze) → fixed to 0.8")
        bad2 = conn.execute("SELECT value FROM tuning_params WHERE key='acc_bb_width'").fetchone()
        if bad2 and float(bad2[0]) < 12.0:
            conn.execute("INSERT OR REPLACE INTO tuning_params VALUES ('acc_bb_width', 20.0)")
            logger.warning(f"[DB REPAIR] acc_bb_width was {bad2[0]} (too tight for hourly) → fixed to 20.0")
