...
# ==========================================
# CHoCH REVERSAL ENGINE — Bounce Play Selepas H4 Breakdown
# ==========================================
class ChochReversalEngine:
    """
    Tangkap bounce play yang berlaku SELEPAS H4 structural breakdown.
    Berbeza sepenuhnya dari AccumulationDetective:

    AccumulationDetective → market sideways/mild bearish → target full reversal
    ChochReversalEngine   → H4 aktif bearish → target bounce ke SBR zone sahaja

    Scenario:
      H4 pecah support besar → harga oversold → M15 tunjuk CHoCH/BOS ↑
      → entry untuk bounce ke level broken support (yang kini jadi resistance)
      → TP1 konservatif, SL tight di bawah swing low M15

    Syarat scoring (5 conditions, dispatch ≥ 3):
      1. H4 bearish  (EMA21 4H — dari SMC context)
      2. RSI H1 oversold (20–42, lebih strict dari Accumulation)
      3. M15 CHoCH ↑ atau BOS ↑ (dari SMC context)
      4. BB Squeeze < 18% (penumpatan pasca-drop, lebih ketat)
      5. Recovery candle (green + higher low pada H1)

    Nota: H4 dan M15 conditions bergantung pada smc_ctx.
    Engine ini MESTI dipanggil selepas SMC pre-fetch dalam layer2_sniper.
    """
    def check(self, ind, t, smc_ctx=None):
        ...
        # 1. H4 bearish dari SMC
        h4_bear  = bool(smc_ctx and smc_ctx.get('htf_4h') == 'BEAR')

        # 2. RSI oversold (lebih rendah threshold vs Accumulation)
        rsi_ok   = 20 < rsi < 42

        # 3. M15 CHoCH/BOS bullish dari SMC
        bos_type = (smc_ctx['bos_choch'].get('type', 'NEUTRAL')
                    if smc_ctx else 'NEUTRAL')
        m15_bull = bos_type in ('CHOCH_BULL', 'BOS_BULL')

        # 4. BB Squeeze (lebih ketat — bounce selepas compression, bukan random drop)
        bb_ok    = bb < 18.0
        ...
        import re as _r
        conditions = {
            f"H4 Bearish Structure":                  h4_bear,
            f"RSI Oversold [{rsi:.0f}]":              rsi_ok,
            f"M15 CHoCH/BOS ↑":                       m15_bull,
            f"BB Squeeze < 18% [{bb:.1f}%]":          bb_ok,
            f"Recovery Candle (Green + HL)":           recovery,
        }

        score = sum(1 for v in conditions.values() if v)

        if score >= 3:
            swing_low = min(ind.lows[-8:])   # SL lebih tight — bounce play
            sig = {
                'type':       'CHOCH_REVERSAL',
                'rvol':       rvol,
                'bb':         bb,
...
def dispatch_signal(symbol, price, sig, ind, engine_type, daily_note,
    ...
    if engine_type == 'SWEEP_REVERSAL':
        ...
        # Bounce play — target realistik ke SBR zone, bukan full reversal.
        # R-multiple sahaja: elak fresh zone yang terlalu jauh untuk bounce.
        tp1 = price + risk * 1.5
        tp2 = price + risk * 3.0
        tp3 = price + risk * 5.0
        # Bonus: kalau OB ada atas entry, jadikan TP1 natural target
        _smc_d = sig.get('smc') or {}
        _ob    = _smc_d.get('ob', {})
        ...
        if len(fresh_h1) >= 2:
            tp3_z = fresh_h1[1]['bot']
        elif fresh_h1:
            tp3_z = fresh_h1[0]['top']
        else:
            tp3_z = swing_high

        # FIX: Enforce minimum R-multiple — fresh zones terlalu rapat dalam downtrend.
        # Ambil yang lebih jauh antara zone dan R-minimum.
        # TP1 ≥ 2R, TP2 ≥ 3.5R, TP3 ≥ 5.5R
        ...
        # Spacing bermakna — min 1.5R antara TP2 dan TP1, 2R antara TP3 dan TP2
    ...
    # ─── UI FIX: ALL conditions ✅/❌ (ganti failed_line satu syarat sahaja) ──
    conditions_block = ""
    if sig.get('conditions'):
        for cname, cpass in sig['conditions'].items():
            short = cname.split('[')[0].strip()[:35]
            short = short.replace('>=', '≥').replace('<=', '≤')
            short = short.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
            conditions_block += f"{'✅' if cpass else '❌'}  {short}\n"

    # ─── UI FIX: BTC macro — satu baris (user request) ───────────────────────
    # Sebelum: "🌐  BTC 🟢 +2.1%"  (tiada 24h Status)
    # Sekarang: "BTC: +2.1% 🟢  24h Status: 🟢"
    btc_line = ""
    if macro_state:
        btc_arrow = "🟢" if macro_state['btc_24h_pct'] >= 0 else "🔴"
        t_thresh  = t.get('macro_btc_24h_min', -2.0)
        ema_ok    = (macro_state.get('btc_above_ema21d', True)
                     if engine_type == 'ACCUMULATION' else True)
        status_ok = macro_state['btc_24h_pct'] >= t_thresh and ema_ok
        status_dot = "🟢" if status_ok else "🔴"
        warn = "  ⚠️" if not macro_ok else ""
        btc_line = (f"BTC: {macro_state['btc_24h_pct']:+.2f}% {btc_arrow}"
                    f"  24h Status: {status_dot}{warn}\n")

    # ─── UI FIX: HTF section conditional — elak double-divider bila tiada SMC ─
    htf_section = f"{htf_line}{SEP}\n" if htf_line else ""

    # ─── NEW MESSAGE FORMAT ───────────────────────────────────────────────────
    # Perubahan: divider ━━━, stars, tiada header "HTF Context/Entry & Risk/
    # Targets/Macro Check:", tiada RR pada TP, BTC sebaris
    msg = (
        f"{emoji}  <code>{symbol}</code>  ●  Nova7  ●\n"
        f"{SEP}\n"
        f"⏱  H1  │  {score}/{score_max}  {stars}\n"
        f"{SEP}\n"
        + htf_section                              # 1D/4H/M15 — tiada header
        ...
        f"{SEP}\n"
        f"{conditions_block}"                      # ✅/❌ semua syarat
        f"{ob_line}"
        f"{fvg_line}"
        f"{reentry_line}"
        f"{SEP}\n"
        f"{btc_line}"                              # BTC: +2.1% 🟢  24h Status: 🟢
        f"{SEP}\n"
                    ...
                    if now - last_scheduled >= 7200:
                        ...
                        async def _staggered_acc_scan(syms):
                            for s in syms:
                                ...
                                # CHoCH Reversal — scan untuk bounce selepas H4 breakdown
                                # Jalan bersama accumulation scan, bukan bergantung pada momentum
                                with queue_lock:
...
async def _layer2_sniper_impl(symbol, scan_type, force, chat_id, user_cap, user_risk):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if scan_type == 'BREAKOUT':
            ...
            # Pre-fetch SMC — ChochReversalEngine memerlukan H4+M15 context
            try:
        ...
        if sig:
            ...
            # SMC Analysis — skip untuk CHOCH (sudah di-fetch sebelum engine check)
            ...
            # 1D🔴 + 4H🔴 serentak = active downtrend, bukan accumulation sebenar.
            # Kurangkan score -1. Kalau jatuh < 3, block (gunakan CHoCH engine
            # untuk market ini). Kalau masih ≥ 3, signal keluar dengan warning.
            if scan_type == 'ACCUMULATION' and sig.get('smc'):
                ...
                if _s.get('htf_1d') == 'BEAR' and _s.get('htf_4h') == 'BEAR':
                    ...
                    if sig['score'] < 3:
                        log_activity(f"{symbol} ❌ ACCUM blocked: HTF double bearish — pakai CHoCH engine")
                        bump_stat('rejected')
