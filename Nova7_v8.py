...
DEFAULT_TUNING = {
    ...
    # Accumulation params — V8 FIX: Threshold dipulihkan ke nilai SQUEEZE sebenar
    'acc_bb_width': 8.0,   # V8: dari 24.0 → 8.0 (real squeeze, bukan high volatility)
    'acc_rvol': 2.0,
    'acc_rsi_max': 45,
    'acc_rsi_min': 25,     # V8 NEW: elak catch capitulation severe (RSI < 25)
    # Radar params
    'radar_momentum': 2.5,
    ...
    'macro_btc_filter': 1,        # 1=on, 0=off
    'macro_btc_24h_min': -1.5,    # BTC 24h % minimum
    # V8 NEW: Confirmation queue (hanya untuk ACCUMULATION)
    'confirm_required': 1,        # 1=on, 0=off
    'pending_expiry_h': 2,        # Pending signal expiry (hours)
    # V8 NEW: SL ATR-based + fail cooldown
    'sl_atr_mult': 1.5,           # ATR multiplier untuk SL buffer
    'sl_max_pct': 0.08,           # Cap: SL tidak lebih dari 8% dari entry
    'fail_cooldown_h': 2,         # Mini cooldown untuk symbol gagal validation
    # V8 NEW: Concurrency limit
    'layer2_concurrency': 5,      # Max concurrent layer2 scans
}
...
class AccumulationDetective:
    """V8 OVERHAUL: Strict accumulation detection.
    Falling knife filter melalui RSI floor + candle direction + higher-low.
    """
    def check(self, ind, t):
        if len(ind.closes) < 51: 
            return None, {}
        ...
        rsi_min = t.get('acc_rsi_min', 25)  # V8: floor untuk elak capitulation
        ...
        conditions = {
            ...
            f"RSI {rsi_min}-{rsi_max} [{ind.rsi:.1f}]": rsi_min < ind.rsi < rsi_max,
            "Candle Hijau (Buying Pressure)": is_green,  # V8 NEW
            "Higher Low (Bounce Hint)": higher_low,       # V8 NEW
        }
        if all(conditions.values()):
            structure_low = min(ind.lows[-20:])
            sig = {
                'type': 'ACCUMULATION', 'rvol': rvol, 'bb': bb,
                'low': structure_low, 'atr': ind.atr,  # V8
            }
            return sig, conditions
        return None, conditions
...
def cmd_tune(msg):
    ...
    if args[0] == 'standard':
        ...
        set_tuning({
            ...
            # V8: macro lebih toleran tapi tidak off
            'macro_btc_filter': 1, 'macro_btc_24h_min': -3.0,
            'confirm_required': 0,  # longgar = skip confirmation untuk lebih signal
