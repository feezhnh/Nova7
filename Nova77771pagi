...
class AccumulationDetective:
    """
    3/4 scoring — signal lulus jika ≥ 3 dari 4 syarat utama dipenuhi.
    Score dan status setiap syarat dihantar dalam mesej Telegram.
    """
    def check(self, ind, t):
        if len(ind.closes) < 51:
            return None, {}
        ...
        score = sum(1 for v in conditions.values() if v)
        # Soft hints — info sahaja, tidak kira dalam score
        hl_hint  = "✓" if higher_low else "–"
        grn_hint = "✓" if is_green   else "–"
        ...
        for idx in range(-1, -(lookback + 1), -1):
            ...
            if abs(idx) + 1 <= n:
                ...
                if c2 < o2 and c > o:           # prev bearish, curr bullish
                    if o <= c2 and c >= o2:     # engulf
                        eng_pct = body / self._range(max(h, highs[idx-1]), min(l, lows[idx-1]))
                        if eng_pct >= 0.60:
                            strength = eng_pct
                            if strength > best['strength']:
                                best = {'pattern': 'ENGULFING', 'strength': round(strength, 3),
                                        'candle_idx': idx, 'found': True}
...
def dispatch_signal(symbol, price, sig, ind, engine_type, daily_note,
    ...
    # MACRO: Amaran sahaja — tidak block signal
    ...
    if engine_type == 'SWEEP_REVERSAL':
        # Structure-based TP — lebih bermakna dari fixed R multiple
        # SL: bawah sweep wick (sudah dikira dalam engine)
        sl = sig.get('sl_raw', compute_final_sl(price, structure_low, atr, t))
        # Cap SL pada 10% max (sweep boleh dalam)
        sl = max(sl, price * 0.90)
        risk = price - sl
        if risk <= 0:
            logger.warning(f"{symbol}: SL >= entry (SWEEP), abort")
            return

        range_high = sig.get('range_high', price * 1.10)
        range_size = sig.get('range_size', risk * 2)

        # TP1: 50% recovery ke range high (target semulajadi pertama)
        tp1 = price + (range_high - price) * 0.50
        # TP2: Range high penuh (logik utama selepas sweep reversal)
        tp2 = range_high
        # TP3: Fibonacci 1.618 extension atas range (jika momentum berterusan)
        tp3 = range_high + (range_size * 0.618)

        # Kira RR sebenar untuk papar dalam mesej
        rr1 = (tp1 - price) / risk if risk > 0 else 0
        rr2 = (tp2 - price) / risk if risk > 0 else 0
        rr3 = (tp3 - price) / risk if risk > 0 else 0
    else:
        # Engines lain kekal guna fixed R multiple
    ...
    elif engine_type == 'SWEEP_REVERSAL':
        emoji, title = "🎣", "SWEEP REVERSAL"
    else:
    ...
    if sig.get('conditions'):
        for cond_name, passed in sig['conditions'].items():
            short = cond_name.split('[')[0].strip()[:35]
            short = short.replace('&amp;', '&').replace('&lt;', '<').replace('&gt;', '>') # unescape dulu
            short = short.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;') # kemudian escape betul
            cond_lines += f"{'✅' if passed else '⚠️'} {short}\n"

    # Desc line
    if engine_type == 'SWEEP_REVERSAL':
        patt = sig.get('pattern', '?')
        pstr = sig.get('pattern_str', 0)
        desc = (f"<b>Range:</b> <code>${sig.get('range_low',0):.6f}</code> – "
                f"<code>${sig.get('range_high',0):.6f}</code>\n"
                f"<b>Sweep Low:</b> <code>${sig.get('sweep_low',0):.6f}</code>  "
                f"<b>Pattern:</b> {patt} {pstr:.0%}")
    elif engine_type in ('BREAKOUT', 'RETEST'):
        lvl  = sig.get('break_level', 0)
        wick = sig.get('wick_ratio', 0)
        desc = f"<b>Level:</b> <code>${lvl:.6f}</code>"
        if engine_type == 'RETEST':
            desc += f"  <b>Wick:</b> {wick*100:.0f}%"
    else:
        desc = f"<b>BB Squeeze:</b> {sig.get('bb', 0):.2f}%"

    atr_val = sig.get('atr', 0)
    atr_pct = (atr_val / price * 100) if price > 0 and atr_val > 0 else 0

    # BTC + Daily — info box, bukan filter
    btc_line = ""
    if macro_state:
        arrow    = "🟢" if macro_state['btc_24h_pct'] >= 0 else "🔴"
        btc_warn = f"  {macro_warning.strip()}" if macro_warning else ""
        btc_line = f"{arrow} BTC {macro_state['btc_24h_pct']:+.2f}%{btc_warn}"

    mode_name = {0: 'STANDARD', 1: 'LONGGAR', 2: 'KETAT'}.get(int(t.get('mode', 0)), 'STD')

    msg = (
        f"{emoji} <b>{title}</b>  <code>{symbol}</code>\n"
        f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
        f"💵 <b>Entry</b>  <code>${price:.6f}</code>\n"
        f"{desc}\n"
        f"📊 <b>Score</b>  <code>{score_bar}</code>\n"
        f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
        f"{cond_lines}"
        f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
        f"🔥 RVOL {sig['rvol']:.2f}x  │  RSI {ind.rsi:.1f}  │  ATR {atr_pct:.2f}%\n"
        f"📈 EMA21 ${ind.ema21:.6f}  │  EMA50 ${ind.ema50:.6f}\n"
    )
    if btc_line:
        msg += f"🌐 {btc_line}\n"
    if daily_note and daily_note != "Filter OFF":
        msg += f"🗓️ Daily: <i>{daily_note}</i>\n"
    msg += (
        f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
        f"🛑 <b>SL</b>  <code>${sl:.6f}</code>  ({((sl-price)/price*100):+.2f}%)\n"
        f"🎯 <b>TP1</b> <code>${tp1:.6f}</code>  +{((tp1-price)/price*100):.1f}%  [{rr1:.1f}R]\n"
        f"🎯 <b>TP2</b> <code>${tp2:.6f}</code>  +{((tp2-price)/price*100):.1f}%  [{rr2:.1f}R]\n"
        f"🎯 <b>TP3</b> <code>${tp3:.6f}</code>  +{((tp3-price)/price*100):.1f}%  [{rr3:.1f}R]\n"
        f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
        f"💼 <b>Lot</b>  {pos_coins:.4f} {symbol[:-4]}  <code>${pos_usd:,.2f}</code>\n"
        f"⚡ <b>Risk</b>  <code>-${risk_usd:,.2f}</code>  ({user_risk}%)\n"
        f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n"
        f"<i>Nova7 · {mode_name}</i>"
    )
...
async def _layer2_sniper_impl(symbol, scan_type, force, chat_id, user_cap, user_risk):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if sig:
            ...
            # daily_ok dihantar ke dispatch untuk papar dalam mesej, bukan untuk block
