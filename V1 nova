...
def dispatch_signal(symbol, price, sig, ind, engine_type, daily_note,
    ...
    # SMC context — hanya ambil data, rendering ada dalam blok UI bawah
    smc_ctx = sig.get('smc')
    ...
    # ─── UI: condition label transformer ──────────────────────────────────────
    import re as _re
    def _fmt_cond_label(key, passed):
        """Transform raw condition key ke label bersih untuk Telegram."""
    ...
    if sig.get('conditions'):
        for cname, cpass in sig['conditions'].items():
            label = _fmt_cond_label(cname, cpass)
            conditions_block += f"{'✅' if cpass else '❌'}  {label}\n"

    # ─── UI: OB/FVG — % sahaja, ✅ jika dalam range ─────────────────────────
    ob_line  = ""
    fvg_line = ""
    if smc_ctx:
        ob  = smc_ctx.get('ob',  {})
        ...
        if fvg.get('found'):
            fvg_in_zone = fvg['bot'] <= price <= fvg['top']
            fvg_emoji   = "✅" if fvg_in_zone else "🌀"
            fvg_line    = f"{fvg_emoji}  FVG   [{fvg['dist_pct']:.1f}%]\n"

    # ─── UI: HTF line — tukar arrow ke colored dot ────────────────────────────
    htf_section = ""
    if smc_ctx:
        ...
        # Convert arrow symbols ke colored dots
        if '↑' in bc_label:
            bc_display = bc_label.replace('↑', '').replace('⚠️','').strip() + '  🟢'
        elif '↓' in bc_label:
            bc_display = bc_label.replace('↓', '').replace('⚠️','').strip() + '  🔴'
        else:
            bc_display = bc_label
        htf_section = f"📐  1D {d1}  4H {h4}  ·  M15 {bc_display}\n{SEP}\n"
    ...
    if macro_state:
        ...
        ema_ok_    = (macro_state.get('btc_above_ema21d', True)
                      if engine_type == 'ACCUMULATION' else True)
        status_ok_ = macro_state['btc_24h_pct'] >= t_thresh and ema_ok_
        status_dot = "🟢" if status_ok_ else "🔴"
        btc_line   = f"BTC: {macro_state['btc_24h_pct']:+.2f}% {btc_arrow}  24h Status: {status_dot}\n"

    # ─── NEW MESSAGE FORMAT (LOCKED) ──────────────────────────────────────────
    msg = (
    ...
    try:
        ...
        # FIX IDEA 4: Selepas BREAKOUT signal dihantar, daftar coin untuk retest monitoring.
        # Ini pastikan RetestHunter dapat track pullback walaupun breakout CD aktif (24h).
        # peak_price = harga signal (akan di-update lebih tinggi oleh layer1_radar jika naik lagi)
        if engine_type == 'BREAKOUT':
            with _retest_lock:
                existing = _retest_watchlist.get(symbol)
                _retest_watchlist[symbol] = {
                    'price_at_promote': price,
                    'peak_price': max(price, existing.get('peak_price', price) if existing else price),
                    'time': time.time(),
                }
            logger.info(f"[RETEST] {symbol} registered post-BO @ ${price:.6f}")
            ...
            async with websockets.connect(url, ping_interval=20, max_size=2*10**6) as ws:  # [MEM-FIX] 10MB→2MB, mini-ticker < 200KB
                ...
                while True:
                    ...
                    for tk in tickers:
                        ...
                        # Track peak price untuk coin dalam retest watchlist (tanpa lock overhead)
                        # Ini pastikan pullback dikira dari HIGH sebenar, bukan harga promote asal
                        with _retest_lock:
                            if sym in _retest_watchlist:
                                _retest_watchlist[sym]['peak_price'] = max(
                                    _retest_watchlist[sym].get('peak_price', c), c)
                            ...
                            if past_c > 0:
                                ...
                                if change >= momentum and q > min_vol and not check_cooldown(sym):
                                    ...
                                    if promote_breakout:
                                        ...
                                        # Tambah ke retest watchlist dengan peak_price
                                        with _retest_lock:
                                            existing = _retest_watchlist.get(sym)
                                            if existing:
                                                # Update peak jika coin dah dalam watchlist
                                                _retest_watchlist[sym]['peak_price'] = max(
                                                    existing.get('peak_price', c), c)
                                            else:
...
async def _layer2_sniper_impl(symbol, scan_type, force, chat_id, user_cap, user_risk):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if sig:
            ...
            # SMC Analysis — guna session yang SAMA (jimat ~1 connection pool per scan)
            # [MEM-FIX] Sebelum: buka ClientSession kedua di dalam session pertama = 2× overhead
            # Sekarang: reuse session luar yang masih dalam context manager
            try:
    ...
    while True:
        ...
        try:
            ...
            for sym, meta in candidates:
                # FIX KRITIKAL: guna RT_ namespace — JANGAN biarkan breakout cooldown (24h)
                # bunuh peluang retest. Retest berlaku DALAM window 24h selepas breakout.
                # RT_ cooldown hanya 2h — cukup untuk elak repeat scan, tidak block peluang.
                if check_cooldown(f"RT_{sym}"):
                    with _retest_lock:
                ...
                # FIX: guna peak_price (harga tertinggi selepas promote), bukan promote price asal.
                # Pullback dari peak = lebih tepat untuk detect RBS zone sebenar.
                promote_price = meta.get('peak_price', meta['price_at_promote'])

                # Update peak price dalam watchlist (track high-water mark)
                with _retest_lock:
                    if sym in _retest_watchlist:
                        _retest_watchlist[sym]['peak_price'] = max(promote_price, cur_price)
